...
void target_verify_accept(ExecutionCore& execution, Tensor& continuation_hidden_store,
    ...
    if (frame.feature_sink != nullptr || peer.feature_sink != nullptr ||
        frame.proposal_q.data != nullptr) {
        throw std::logic_error(
            "tensor-parallel target verify supports greedy-draft acceptance without feature "
            "capture");
    }
    card.set_gdn_state_action(GdnStateAction::RecordForReplay, frame.replay_records);
    card.target_verify_batch(
        ...
        {frame.state_source_slots, peer.state_source_slots}, envelope,
        TextContext::RankTensors{frame.target_hidden, peer.target_hidden},
        TextContext::RankTensors{frame.target_logits, peer.target_logits}, frame.target_tokens);
    ops::speculative_accept_greedy_drafts(
        frame.target_tokens, frame.target_logits, frame.drafts, frame.current_extents,
        frame.frontiers, frame.anchors, frame.licensed_tokens, frame.licensed_counts,
        frame.accepted_drafts, dimension(execution.parameters.model.resources().public_token_count),
