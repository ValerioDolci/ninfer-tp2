...
// threshold; they have not been re-measured at this shape. Its W4A4 floor is instead the fused
// residual projection's (nvfp4_linear_add_plan.cpp), not linear()'s 8: at tp 2 rank 1 runs this
// linear() and rank 0 runs linear_add over the other half of the same row-parallel projection, so
// the two ranks take the same activation route at every width, as tp 1's single linear_add
// [5120,6144] does.
namespace {
