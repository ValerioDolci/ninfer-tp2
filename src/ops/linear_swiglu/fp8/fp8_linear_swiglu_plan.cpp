...
void launch_a16(const Tensor& x, const Weight& weight, Tensor& out, cudaStream_t stream) {
    const std::int32_t output_rows = weight.n / 2;
    ...
    for (std::int32_t token_begin = 0; token_begin < x.ne[1]; token_begin += kChunk) {
        ...
        auto* output = static_cast<std::uint8_t*>(out.data) +
                       static_cast<std::int64_t>(token_begin) * output_rows * sizeof(std::uint16_t);
        Tensor input_chunk(input, DType::BF16, {weight.k, active});
        Tensor output_chunk(output, DType::BF16, {output_rows, active});
        if (active == 1) {
